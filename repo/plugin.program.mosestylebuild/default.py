# Mosestyle Build Installer — safer skin-switch + restart disabled by default
...
def wait_for_skin_prompt(timeout=120):
    ...
    while time.time() < end and not mon.abortRequested():
        if skin_prompt_active():
            ...
            if saw_prompt:
                return True  # was visible and now gone
        xbmc.sleep(250)
    return not saw_prompt  # True if we never saw it
    ...
    try:
        ...
        if ADDON.getSettingBool('auto_set_skin'):
            ...
            if target_skin:
                ...
                # Wait for the "Keep skin?" prompt to finish before any final dialogs/restart
        ...
        # default is OFF; user can enable if they want
