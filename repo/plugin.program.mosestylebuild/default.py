# Mosestyle Build Installer — safer skin-switch + restart guards
...
def main():
    ...
    try:
        ...
        if ADDON.getSettingBool('auto_set_skin'):
            ...
            log("auto_set_skin is OFF (skipping skin switch)")

        xbmcgui.Dialog().ok('Mosestyle Build',
                            "Build installed.\nOn next start, Mosestyle's First-Run Config will apply GUI settings.")

        # Extra guard: never restart if skin prompt is (or was) active
        if ADDON.getSettingBool('auto_restart'):
            if not skin_prompt_active():
                xbmc.executebuiltin('RestartApp')
            else:
                log("auto_restart requested but skin prompt is/was active; skipping restart for safety")
